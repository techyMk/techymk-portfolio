...
export default function Hero() {
  ...
  return (
    <section id="home" className="relative min-h-screen flex flex-col overflow-hidden">
      ...
      <div ref={ref} className="max-w-content mx-auto w-full px-6 pt-28 md:pt-32 pb-6 flex-1 flex flex-col justify-center relative z-10">

        {/* 3-col grid: text | photo | text */}
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px_1fr] gap-4 lg:gap-6 items-center">

          {/* Left column — flush right (towards image), TECHY MK above WEBSITE's left edge */}
          <motion.div className="relative min-w-0 flex items-center justify-end" style={{ x: textPar }}>
            <div className="relative inline-block">
              <motion.p
                className="absolute bottom-full left-0 mb-2 text-content-primary text-[11px] md:text-[13px] tracking-[0.25em] uppercase font-bold whitespace-nowrap"
              ...
              <h1 className="font-display font-black text-content-primary uppercase leading-none tracking-[-0.04em] whitespace-nowrap text-[clamp(36px,5.5vw,80px)]">
          ...
          >
            <motion.div className="relative" style={{ x: photoX, y: photoY }}>
              ...
              />
              {/* Photo — replace src with your actual portrait image */}
              <div className="relative w-full h-[330px] md:h-[360px] lg:h-[390px] rounded-[24px] overflow-hidden border border-border/30 shadow-2xl shadow-black/25">
                <img
                  src="assets/hero.png"
                  alt="Profile"
              ...
              {/* Hi / Hand badge */}
              <motion.div
                className="absolute -bottom-4 -left-4 w-[72px] h-[72px] md:w-[90px] md:h-[90px] rounded-full bg-accent flex items-center justify-center shadow-xl shadow-accent/30"
              ...
              >
                <AnimatePresence mode="wait">
                  {showWave ? (
                    <motion.div key="wave" className="w-[30px] h-[30px] md:w-[38px] md:h-[38px]" style={{ originX: 0.5, originY: 0.9 }} initial={{ opacity: 0, scale: 0.4 }} animate={{ opacity: 1, scale: 1, rotate: [0, 14, -8, 14, -4, 10, 0, 0, 14, -8, 14, -4, 10, 0] }} exit={{ opacity: 0, scale: 0.4 }} transition={{ duration: 0.3, rotate: { duration: 2.4, delay: 0.2, ease: 'easeInOut' } }}>
                      <img src="https://cdn.jsdelivr.net/gh/twitter/twemoji@latest/assets/svg/270b.svg" alt="wave" className="w-full h-full" />
                    </motion.div>
                  ) : (
                    <motion.span key="hi" className="text-white font-display text-[20px] md:text-[26px] font-medium" initial={{ opacity: 0, scale: 0.4 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.4 }} transition={{ duration: 0.35 }}>Hi!</motion.span>
                  )}
                </AnimatePresence>
              </motion.div>
            </motion.div>
          ...
          <motion.div className="relative min-w-0 flex items-center justify-start" style={{ x: textPar }}>
            <div className="relative inline-block">
              <h1 className="font-display font-black text-content-primary uppercase leading-none tracking-[-0.04em] whitespace-nowrap text-[clamp(36px,5.5vw,80px)]">
                <CharReveal delay={0.8} inView={inView}>DEVELOPER</CharReveal>
              </h1>
              <motion.p
                className="absolute top-full right-0 mt-2 text-content-secondary text-[12px] md:text-[14px] leading-relaxed text-right whitespace-nowrap"
        ...
        >
          <RippleButton href="#projects" rippleClass="bg-accent" className="inline-flex items-center gap-2 px-7 py-3 rounded-full bg-black text-white dark:bg-white dark:text-black dark:hover:text-white text-[13px] font-semibold shadow-lg">
            View My Work <ArrowDown size={15} />
          </RippleButton>
          <RippleButton href="#contact" rippleClass="bg-accent" className="group inline-flex items-center gap-2 px-7 py-3 rounded-full border border-accent text-accent text-[13px] font-semibold hover:text-white transition-colors">
            Hire Me <ArrowUpRight size={15} />
          </RippleButton>
        </motion.div>

        {/* Scroll */}
        <motion.div className="flex justify-center mt-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.7 }}>
      ...
      {/* Marquee */}
      <div className="border-t border-border/30 py-3 overflow-hidden mt-auto">
        <div className="flex animate-marquee whitespace-nowrap">
          {[...marqueeItems, ...marqueeItems].map((w, i) => (
            <span key={i} className="flex items-center gap-8 md:gap-12 mx-8 md:mx-12 font-display text-[24px] md:text-[36px] font-semibold uppercase select-none">
